...
public class ArrayAccess implements ArrayAccessIf {
	 ...
	 */
	@Override
	public void initArray(String[] initialArray) {	
		try
		{	

//			ArrayForCompare[count] = Integer.parseInt(initialArray[count]);
			if (initialArray.length > 10)
			{
			...
			for (int count = 0; count < initialArray.length; count++)
			{
				...
				// check each values with two loops 
				for (int j = 0; j < initialArray.length; j++)
				{
					
					if (arrayForAccess[count] == Integer.parseInt(initialArray[j])) {
